...
if nargin < 1
    tmpAllGenes = Pej_Read_Table('/Users/pmohammadi/Desktop/LocalTMP/PEJ_Resources/Gene-Xref-25Apr2015.txt', '\t', false);
...
for dbFile = 1:length(listing)
    ...
    if ~isempty(FName) && FName(1)~= '.' && strcmpi(Fext, '.gmt')
        ...
        if ~feof(F)
            warning('Yo the file was not imported completely! Check for invalid line return chars.')
        end
        fclose(F);
