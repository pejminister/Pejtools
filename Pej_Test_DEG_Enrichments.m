...
for f = 1 : length(Resultfiles)
    if ~isempty(Resultfiles)
       [~, Fname, Fext] =fileparts(Resultfiles(f).name);
        ...
        Test_DEGs_Enrichments_of(DEGoutputfolder, Fname,DB_Path, Shuffle, Qthr, CFthr);
    end
end
end


function Test_DEGs_Enrichments_of(Fldr, Fname,DB_Path, Shuffle, Qthr, CFthr)
if nargin<6
    CFthr=0;
end
...
IDXKM(:,1)= Qvals<Qthr & CF>CFthr;

Clustered_Genes.ClusterLabels2{2} = ['Dnregulated'];
IDXKM(:,2)= Qvals<Qthr & CF<-CFthr;
