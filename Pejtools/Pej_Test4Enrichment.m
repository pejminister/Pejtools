...
if nargin < 2
    DB_Path = '~/Desktop/LocalTMP/PEJ_Resources/GeneSets/';
end

if nargin < 3
    XrefPath = '~/Desktop/LocalTMP/PEJ_Resources/Gene-Xref-16Mar2016.txt';
end
...
if isempty(ReferenceFiles)
    error('No reference genesets found to look for enrichments!')
end
...
for SourceDB = 1:length(ReferenceFilesLabels)
    ...
    if isvector(Clustered_Genes.Cluster_IDs) && isfield(Clustered_Genes, 'Kchoice')
    ...
    Input.ClusterNames = Clustered_Genes.ClusterLabels2;
    ...
    Results.ExpCountsInClusters = Ex_P1;
