...
for i = NinF:-1:1
    ...
    for j = 2:size(inFiles,1)
        ...
        D(i).Results = Pej_Struct_Cat(D(i).Results, DH.Results);
...
%% Expand MultiCluster Files!
tmpdC=1;
for i = 1:NinF
   for j = 1:size(D(i).Results.Q_val,2)
   DE(tmpdC).Input                          = D(i).Input;
   DE(tmpdC).Input.Clusters                 = D(i).Input.Clusters(:,j);
   DE(tmpdC).Input.ClusterNames             = D(i).Input.ClusterNames(j);

      
   DE(tmpdC).Results.GenesInClust           = D(i).Results.GenesInClust(:,j);
   DE(tmpdC).Results.meancoeffs             = D(i).Results.meancoeffs(:,j);
   DE(tmpdC).Results.OverUnderRepres        = D(i).Results.OverUnderRepres(:,j);
   DE(tmpdC).Results.SetCountsInBackground  = D(i).Results.SetCountsInBackground;
   DE(tmpdC).Results.SetCountsInClusters    = D(i).Results.SetCountsInClusters(:,j);
   DE(tmpdC).Results.P_val                  = D(i).Results.P_val(:,j);
   DE(tmpdC).Results.Q_val                  = D(i).Results.Q_val(:,j);
   DE(tmpdC).Results.P_val(:,2)             = 1;
   DE(tmpdC).Results.Q_val(:,2)             = 1;

   FnameE{tmpdC} = [Fname{i} '-' D(i).Input.ClusterNames{j}];
   tmpdC = tmpdC+1;
   end 
end
...
SGS.SuperSet = WhosYourDaddy(SGS.Genelist,Ndaddies);
...
for i = 1:NinF
    ...
    text(zeros(1,7)-0.025 -.01, -3:3, num2cell(-3:3), 'HorizontalAlignment', 'right', 'FontSize', 14)
    % Right Side
    plot(ones(1,7)*BorderCords(end), -3:3, 'k+','linewidth', 1)
    plot([1 1]    *BorderCords(end), [-3.25 +3.25], 'k','linewidth', 1.2)
    text(ones(1,7)*BorderCords(end)+0.05, -3:3, num2cell(-3:3), 'HorizontalAlignment', 'right', 'FontSize', 14)
